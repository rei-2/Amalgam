...
MAKE_SIGNATURE(CTFClientScoreBoardDialog_OnScoreBoardMouseRightRelease_AddMenuItem_Call, "client.dll", "48 8B 0D ? ? ? ? 4C 8D 84 24 ? ? ? ? 48 8D 94 24 ? ? ? ? 48 8B 01 FF 90 ? ? ? ? 44 8B 84 24 ? ? ? ? 8B 94 24", 0x0);
    ...
    void* rcx, const char* pszButtonText, const char* pszCommand, const char* pszCategoryName)
{
    ...
    if (Vars::Visuals::UI::ScoreboardUtility.Value && dwRetAddr == dwDesired && PlayerIndex != -1)
    {
        ...
        if (I::EngineClient->GetPlayerInfo(PlayerIndex, &pi) && !pi.fakeplayer)
        {
            ...
            const bool bIgnored = F::PlayerUtils.HasTag(FriendsID, IGNORED_TAG);
            const bool bCheater = F::PlayerUtils.HasTag(FriendsID, CHEATER_TAG);
    ...
    void* rcx, const char* command)
{
    ...
    case FNV1A::Hash32Const("ignoreplayer"):
        if (!F::PlayerUtils.HasTag(FriendsID, IGNORED_TAG))
            F::PlayerUtils.AddTag(FriendsID, IGNORED_TAG, true, PlayerName);
        else
            F::PlayerUtils.RemoveTag(FriendsID, IGNORED_TAG, true, PlayerName);
        break;
    case FNV1A::Hash32Const("markplayer"):
        if (!F::PlayerUtils.HasTag(FriendsID, CHEATER_TAG))
            F::PlayerUtils.AddTag(FriendsID, CHEATER_TAG, true, PlayerName);
        else
            F::PlayerUtils.RemoveTag(FriendsID, CHEATER_TAG, true, PlayerName);
        break;
