...
inline Color_t GetScoreboardColor(int iIndex)
{
	Color_t out = { 0, 0, 0, 0 };
	...
	if (out.a)
	{
		...
		if (pResource && !pResource->IsAlive(iIndex))
			out = out.Lerp({ 127, 127, 127, out.a }, 0.5f);
	}
...
{
	if (Vars::Visuals::UI::ScoreboardColors.Value)
	{
		Color_t tColor = GetScoreboardColor(iPlayerIndex);
		if (tColor.a)
			color = tColor;
	}
